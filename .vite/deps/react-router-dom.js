import { require_react_dom } from './chunk-R3E4LXVO.js';
...
function RouterProvider({
  ...
  let setState = React3.useCallback(
    (
      newState,
      { deletedFetchers, newErrors, flushSync: flushSync3, viewTransitionOpts }
    ) => {
            ...
            React3.startTransition(() => {
              if (unstable_useTransitions === true) {
                setOptimisticState((s) =>
                  getOptimisticRouterState(s, newState)
                );
              }
              setStateImpl(newState);
            });
...
try {
  if (isBrowser2) {
    window.__reactRouterVersion = '7.11.0'; // @ts-expect-error
  }
} catch (e) {}
